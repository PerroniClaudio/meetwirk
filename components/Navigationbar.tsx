...
export function NavigationBar() {
  ...
  return(
    <div className="flex gap-0">
      <a href="/" className="absolute left-0 top-2 ml-2 px-4 flex justify-center items-center">
        <img src="/images/favicon-ift-bianco.webp" alt="logo" className="w-8 h-8 z-[51]" />
      </a>
      
      <div className="text-black font-semibold flex flex-row grow justify-center">

          <a href="/" 
            className={`group select-none relative self-center h-full p-3 ${pathId === "/" ?  "text-gray-50 font-extrabold" : ""} hover:bg-torch-600 transition-colors duration-200`}>
            <span>HOME</span>
            <span className="pointer-events-none w-80 text-center bg-gray-50 p-1 mx-auto -mb-2 text-sm text-gray-900 rounded-md opacity-0 z-10
        ...
        {components.map((component, index) => (
            <a key={component.title} href={component.href} 
              className={`group select-none relative self-center h-full p-3 ${pathId === component.href ?  "text-gray-50 font-extrabold" : ""} hover:bg-torch-600 transition-colors duration-200`}
            >
              ...
              <span className={`pointer-events-none w-80 text-center bg-gray-50 p-1 mx-auto -mb-2 text-sm text-gray-900 rounded-md opacity-0 z-10
                absolute bottom-0 translate-y-full ${index < (components.length - 1) ? "left-1/2 -translate-x-1/2 " : "right-0" }
                hover:hidden group-hover:opacity-100 transition-opacity`}
              >
                {component.description.length > 60 ? component.description.substring(0, 60) + "..." : component.description}
              </span>
